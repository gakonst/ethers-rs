...
impl AbiParser {
    ...
    /// type if `type_str` is not uint8. However can lead to some problems if a function param is array of custom types for example, like `Foo[]`, which the `Reader` would identify as `uint8[]`. Therefor if the `Reader` returns an `uint8` we also check that the input string contains a `uint8`. This however can still lead to false detection of `uint8` and is only solvable with a more sophisticated parser: https://github.com/gakonst/ethers-rs/issues/474
...
pub fn contains_uint8(kind: &ParamType) -> bool {
    match kind {
        ...
        ParamType::Array(arr) => contains_uint8(&*arr),
        ParamType::FixedArray(arr, _) => contains_uint8(&*arr),
