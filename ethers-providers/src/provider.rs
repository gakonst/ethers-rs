...
impl<P: JsonRpcClient> Provider<P> {
    pub fn call_builder<'a>(&'a self, tx: &'a TypedTransaction) -> Caller<'a, P> {
        Caller::Build(CallBuilder::new(self, tx))
    }
}
...
pub struct CallInput<'a> {
    ...
    block: Option<BlockId>,
    spoof: Option<&'a spoof::State>,
}
impl<'a> CallInput<'a> {
    pub fn new(tx: &'a TypedTransaction) -> Self {
        Self { tx, block: None, spoof: None }
    }
}
...
impl<'a> Serialize for CallInput<'a> {
    ...
    {
        let len = 2 + self.spoof.is_some() as usize;
...
impl<'a, P> CallBuilder<'a, P> {
    fn new(provider: &'a Provider<P>, tx: &'a TypedTransaction) -> Self {
        Self { provider, input: CallInput::new(tx) }
    }
}
impl<'a, P: JsonRpcClient> CallBuilder<'a, P> {
...
impl<'a, P> Caller<'a, P> {
    ...
    pub fn spoof(self, state: &'a spoof::State) -> Self {
        self.map(|mut call| call.input.spoof = Some(state))
    }
    pub fn unwrap_builder(self) -> CallBuilder<'a, P> {
        match self {
            Self::Build(b) => b,
            _ => panic!("Caller::unwrap_builder on a Wait value"),
        }
    }
}

impl<'a, P: JsonRpcClient> Future for Caller<'a, P> {
...
mod tests {
    ...
    impl<'a> From<&'a CallInputOwned> for CallInput<'a> {
        fn from(src: &'a CallInputOwned) -> Self {
            Self { tx: &src.0, block: src.1, spoof: src.2.as_ref() }
        }
    }

    // Tests "roundtrip" serialization of calls: deserialize(serialize(call)) == call
    fn test_encode<'a, P>(call: Caller<'a, P>) {
        let input = call.unwrap_builder().input;
        ...
        assert_eq!(input.spoof, de.spoof);
    ...
    fn test_serialize_call() {
        let adr1: Address = "0x6fC21092DA55B392b045eD78F4732bff3C580e2c".parse().unwrap();
        let adr2: Address = "0x295a70b2de5e3953354a6a8344e616ed314d7251".parse().unwrap();
        let h1 = utils::keccak256("foo").into();
        let h2 = utils::keccak256("bar").into();

        let tx = TypedTransaction::default();
        let (provider, _) = Provider::mocked();

        let call = provider.call_builder(&tx);
        test_encode(call);
        ...
        state
            ...
            .store(h1, 534.into())
            .store(h2, 35.into());
        let call = provider.call_builder(&tx).block(100.into()).spoof(&state);
        test_encode(call);

        let mut state = spoof::state();
        state.account(adr1).nonce(1.into());
        state.account(adr2).nonce(7.into());
        let call = provider.call_builder(&tx).spoof(&state);
        test_encode(call);
