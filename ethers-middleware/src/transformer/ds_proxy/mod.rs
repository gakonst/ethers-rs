...
/// The function signature of DsProxy's execute function, to execute data on a
/// target address.
...
/// The function signature of DsProxy's execute function, to deploy bytecode and
/// execute data on it.
...
#[derive(Debug, Clone)]
/// Represents the DsProxy type that implements the
/// [Transformer](super::Transformer) trait.
///
/// # Example
///
/// ```no_run
...
impl DsProxy {
    /// Create a new instance of DsProxy by providing the address of the DsProxy
    /// contract that has already been deployed to the Ethereum network.
...
impl DsProxy {
    ...
    pub async fn build<M: Middleware, C: Into<Arc<M>>>(
        ...
        if tx_receipt.status == Some(U64::from(1u64)) {
            // fetch the appropriate log. Only one event is logged by the DsProxyFactory
            // contract, the others are logged by the deployed DsProxy contract
...
impl DsProxy {
    /// Execute a tx through the DsProxy instance. The target can either be a
    /// deployed smart contract's address, or bytecode of a compiled smart
    /// contract. Depending on the target, the appropriate `execute` method
    /// is called, that is, either [execute(address,bytes)](https://github.com/dapphub/ds-proxy/blob/master/src/proxy.sol#L53-L58)
