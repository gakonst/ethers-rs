...
async fn gas_escalator_live() {
    ...
    // broadcast 3 txs
    drop(provider.send_transaction(tx.clone().nonce(nonce), None).await.unwrap());
    drop(provider.send_transaction(tx.clone().nonce(nonce + 1), None).await.unwrap());
    drop(provider.send_transaction(tx.clone().nonce(nonce + 2), None).await.unwrap());
