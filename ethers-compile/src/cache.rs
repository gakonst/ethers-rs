...
impl SolFilesCache {
    ...
    pub fn write(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        let file = fs::File::create(path).map_err(|err| CompilerError::io(err, path))?;
        tracing::trace!(
...
impl SolFilesCache {
    pub async fn async_read(path: impl AsRef<Path>) -> Result<Self> {
        let path = path.as_ref();
        let content =
            tokio::fs::read_to_string(path).await.map_err(|err| CompilerError::io(err, path))?;
    ...
    pub async fn async_write(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        tokio::fs::write(path, content).await.map_err(|err| CompilerError::io(err, path))
    }
}
...
impl CacheEntry {
    ...
    pub fn read_last_modification_date(file: impl AsRef<Path>) -> Result<u64> {
        ...
        let last_modification_date = fs::metadata(file)
            .map_err(|err| CompilerError::io(err, file.to_path_buf()))?
            .modified()
            .map_err(|err| CompilerError::io(err, file.to_path_buf()))?
            .duration_since(UNIX_EPOCH)
            .map_err(|err| CompilerError::solc(err.to_string()))?
            .as_millis() as u64;
