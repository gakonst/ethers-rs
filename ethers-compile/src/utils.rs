...
pub fn installed_versions(root: impl AsRef<Path>) -> Result<Vec<Version>, CompilerError> {
...
pub fn read_json_file<T: DeserializeOwned>(path: impl AsRef<Path>) -> Result<T, CompilerError> {
    let path = path.as_ref();
    let file = std::fs::File::open(path).map_err(|err| CompilerError::io(err, path))?;
...
pub fn create_parent_dir_all(file: impl AsRef<Path>) -> Result<(), CompilerError> {
