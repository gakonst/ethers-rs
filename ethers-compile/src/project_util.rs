...
use crate::{
    ...
    hh::HardhatArtifacts,
    utils::tempdir,
    ArtifactOutput, CompilerIoError, ConfigurableArtifacts, PathStyle, Project,
...
impl<T: ArtifactOutput> TempProject<T> {
    ...
    pub fn create_new(
        root: TempDir,
        inner: Project<T>,
    ) -> std::result::Result<Self, CompilerIoError> {
    ...
    fn get_lib(&self) -> Result<PathBuf> {
        self.paths()
            ...
            .get(0)
            .cloned()
            .ok_or_else(|| CompilerError::msg("No libraries folders configured"))
    }
...
fn create_contract_file(path: PathBuf, content: impl AsRef<str>) -> Result<PathBuf> {
    ...
    std::fs::write(&path, content.as_ref())
        .map_err(|err| CompilerIoError::new(err, path.clone()))?;
    Ok(path)
}
...
pub fn copy_file(source: impl AsRef<Path>, target_dir: impl AsRef<Path>) -> Result<()> {
    ...
    let target = target_dir.as_ref().join(
        source
            .file_name()
            .ok_or_else(|| CompilerError::msg(format!("No file name for {}", source.display())))?,
    );
