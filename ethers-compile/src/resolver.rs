...
use crate::{error::Result, utils, ProjectPathsConfig, Solc, CompilerError, Source, Sources};
...
impl Graph {
    ...
    fn get_input_node_versions(
        ...
        if errors.is_empty() {
            ...
            Err(CompilerError::msg(errors.join("\n")))
        }
    }
...
fn read_node(file: impl AsRef<Path>) -> Result<Node> {
    ...
    let source = Source::read(file).map_err(CompilerError::Resolve)?;
