...
impl ArtifactOutput for ConfigurableArtifacts {
    ...
    fn write_contract_extras(&self, contract: &Contract, file: &Path) -> Result<(), CompilerError> {
...
impl ExtraOutputFiles {
    ...
    pub fn write_extras(&self, contract: &Contract, file: &Path) -> Result<(), CompilerError> {
        if self.metadata {
            if let Some(ref metadata) = contract.metadata {
                ...
                fs::write(&file, serde_json::to_string_pretty(metadata)?)
                    .map_err(|err| CompilerError::io(err, file))?
            }
        }
        ...
        if self.ir_optimized {
            if let Some(ref iropt) = contract.ir_optimized {
                ...
                fs::write(&file, iropt).map_err(|err| CompilerError::io(err, file))?
            }
        }
        ...
        if self.ewasm {
            if let Some(ref ir) = contract.ir {
                ...
                fs::write(&file, ir).map_err(|err| CompilerError::io(err, file))?
            }
        }
        ...
        if self.assembly {
            if let Some(ref evm) = contract.evm {
                if let Some(ref asm) = evm.assembly {
                    ...
                    fs::write(&file, asm).map_err(|err| CompilerError::io(err, file))?
                }
            }
        }
