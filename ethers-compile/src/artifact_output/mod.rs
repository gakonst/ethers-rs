...
impl<T: Serialize> ArtifactFile<T> {
    ...
    pub fn write(&self) -> Result<()> {
        ...
        fs::write(&self.file, serde_json::to_vec_pretty(&self.artifact)?)
            .map_err(|err| CompilerError::io(err, &self.file))?;
        Ok(())
    }
}
...
pub trait Artifact {
        ...
        Self: Sized,
    {
        ...
        Ok((
            abi.ok_or_else(|| CompilerError::msg("abi missing"))?,
            bytecode.ok_or_else(|| CompilerError::msg("bytecode missing"))?,
            deployed_bytecode.ok_or_else(|| CompilerError::msg("deployed bytecode missing"))?,
        ))
    }
}
...
pub trait ArtifactOutput {
    ...
    fn read_cached_artifact(path: impl AsRef<Path>) -> Result<Self::Artifact> {
        ...
        let file = fs::File::open(path).map_err(|err| CompilerError::io(err, path))?;
...
impl ArtifactOutput for MinimalCombinedArtifactsHardhatFallback {
    ...
    fn read_cached_artifact(path: impl AsRef<Path>) -> Result<Self::Artifact> {
        let path = path.as_ref();
        let content = fs::read_to_string(path).map_err(|err| CompilerError::io(err, path))?;
