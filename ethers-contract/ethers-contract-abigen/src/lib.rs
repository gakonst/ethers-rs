...
use ethers_solc::artifacts::{ContractBytecode, CompactContract};
...
impl Abigen {
    ...
    /// Generates the contract bindings using go-ethereum's `abigen`
    pub fn go_generate(self) -> Result<GoContractBindings> {
        let name = self.contract_name.clone();
        let bindings = self.go_abigen()?;
        Ok(GoContractBindings { bindings, name })
    }
    ...
    /// Runs go-ethereum's abigen to generate contract bindings
    fn go_abigen(self) -> Result<Vec<u8>> {
        let name = self.contract_name.clone();
        let abi_str = self.abi_source.get()?;
        let contract: ContractBytecode = serde_json::from_str(&abi_str)?;

        // TODO: handle linking
        let compact: CompactContract = contract.into();
        let (abi, bin, _) = compact.into_parts_or_default();
        ...
        abi_file.write_all(&raw_abi)?;

        let output = Command::new("abigen")
            .arg("--bin")
            .arg(bin_path)
            .arg("--abi")
            .arg(abi_path)
        ...
        if !output.status.success() {
            return Err(eyre::format_err!("abigen exit code not success"))
        }
...
/// Represents a Go contract bindings
#[derive(Debug, Clone)]
pub struct GoContractBindings {
    bindings: Vec<u8>,
    name: String,
}

impl GoContractBindings {
    /// Writes the bindings to a given `Write`.
    pub fn write<W>(&self, mut w: W) -> Result<()>
    where
        W: Write,
    {
        w.write_all(&self.bindings)?;
        Ok(())
    }
...
"#,
    ...
    #[ignore]
    fn can_generate_go() {
        let greeter = include_str!("../../tests/solidity-contracts/greeter.json");
        let abigen = Abigen::new("Greeter", greeter).unwrap();
        let gen = abigen.go_generate().unwrap();
        let out = String::from_utf8(gen.bindings).unwrap();

        let json: serde_json::Value = serde_json::from_str(greeter).unwrap();
        let bin = json["bytecode"]["object"].as_str().unwrap();

        assert!(out.contains(bin));
        assert!(out.contains("package Greeter"));
    }
}
